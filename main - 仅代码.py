...
def create_gis():
    ...
    df_phy_stop['Stop_Name'] = df_phy_stop['Stop_Name'].str.replace('公交站', '')
    df_phy_stop['Stop_Name'] = df_phy_stop['Stop_Name'].str.replace(')', '', regex=True)
        ...
        for j in range(df_stop_selected.shape[0]):
           ...
           # 如果能匹配到，即order不为空，则更新轨迹列表，并根据轨迹前后位置计算站点的方位角，将站点信息存起来
            if order:
                ...
                # df_stop_current=df_stop_current.to_frame()#YM修改
                # df_stop_current = pd.DataFrame(df_stop_current.values.T, index=df_stop_current.columns, columns=df_stop_current.index)#YM修改
                # df_stop = pd.concat([df_stop,df_stop_current])#YM修改
    ...
    arr_xy = np.asarray(arr_xy, 'float64')
    ...
    arr_xy = df_stop[['Lng', 'Lat']].values
    for i in range(df_stop.shape[0]):
        lst_wgs_x.append(coord_convert.gcj02_to_wgs84(arr_xy[i][0],arr_xy[i][1])[0])
        lst_wgs_y.append(coord_convert.gcj02_to_wgs84(arr_xy[i][0],arr_xy[i][1])[1])
...
def get_arrival_time(date):
    ...
    for i in range(df_index_line.shape[0]):
        ...
        df_gps_match=pd.concat([df_gps_match,df_gps_selected_match])#YM修改
...
def get_on_stop(date):
    ...
    df_transaction = pd.merge(df_transaction, df_index_line, left_on='Line_ID', right_on='Line_ID_Transaction',
                              how='inner')
    df_transaction.drop(df_transaction[(df_transaction['Card_ID'] == 0) |# ~(df_transaction['Card_Type'].isin(lst_match_card_type)) |
        (df_transaction['Bus_ID'] == 0) | ( df_transaction['Line_ID'] == 0)].index, inplace=True)
    df_transaction.drop(df_transaction[(df_transaction['Card_ID'] == 0) | (df_transaction['Bus_ID'] == 0) | (
                                                   df_transaction['Line_ID'] == 0)].index, inplace=True)#YM修改
    ...
    for i in range(len(arr_bus_id)):
        ...
        df_transaction_on = pd.concat([df_transaction_on,df_transaction_selected_match])#YM修改
...
def get_on_statictics():
    ...
    for k in range(len(lst_date)):
        ...
        df_on_user_stop_date=pd.concat([df_on_user_stop_date,df_temp])#YM修改
...
def get_off_stop(date):
    ...
    # 如果用户为低频乘客，根据所有乘客在该线路当天下游车站上客量确定
    ...
    df_transaction_off['Off_Timestamp'] = df_transaction_off['Timestamp'].apply(_time_str_to_timestamp) + (df_transaction_off['Cost'] * 60).astype(int)#ym修改
...
def get_od():
    ...
    for k in range(len(lst_date)):
        ...
        # df_transaction['Time_Interval'] = df_transaction['Timestamp'].apply(_time_str_to_timestamp).apply(_timestamp_to_time_interval)#YM修改
        ...
        df_od1 = pd.concat([df_od1,df_od1_date])# YM修改
        ...
        df_od2=pd.concat([df_od2,df_od2_date])# YM修改
...
def main():
    ...
    # 指定起止日期，1-9月份前面要有0
    date_start = '2021.09.01'#eg'2021.05.10','2019.04.05'
    date_end = '2021.09.30'
    ...
    lst_match_card_type = [0, 3, 4, 6, 10, 11, 101]                    # 有效交易类型：2 - 异型卡；3 - 纪念卡；4 - 学生卡；6 - 老人卡；10 - 离休卡；11 - 老年人免费卡：101 - 正常卡
    ...
    max_gps2stop_dist = 0.05                                           # GPS与站点匹配距离上限，单位公里，默认0.01公里
    ...
    statistics(lst_date[0])
    assignment(lst_date[0])                               # Step9：公交分配
